...
def run_evaluation(test_file: str, output_file: str = "output/eval_result.xlsx") -> None:
    ...
    Prints QWK, Adjacent Accuracy, and MAE.
    输出 QWK、相邻准确率和 MAE。
    Saves a detailed Excel report.
    保存详细的 Excel 报告。
    """
    from config import FINAL_RULE_FILE, SCORE_MIN, SCORE_MAX
    ...
    # Score / 评分
    print(f"[Eval] Scoring {len(essays)} essays...")
    if TASK_MODE == "unstructured":
        records = cognitively_aligned_scoring(rule, essays, anchor_text)
    else:
        records = run_sa(rule, essays, anchor_text)

    # Metrics / 指标
    qwk, details = calculate_qwk_detailed(essays, records)
    print("\n" + "=" * 50)
